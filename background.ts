// Listen for content scripts request for ADO oauth token
chrome.runtime.onMessage.addListener((message, sender, callback) => {
  chrome.cookies
    .get({ url: message.domain, name: message.name })
    .then(callback);
  return true;
});
...
chrome.webNavigation.onHistoryStateUpdated.addListener(function (details) {
  if (
    regex.test(details.url) &&
    details.frameId == 0 &&
    transitionTypes.includes(details.transitionType)
  ) {
